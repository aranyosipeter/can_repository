...
void balancingTank(){
    ...
    if ( balancingMenu ){
      ...
      conkI = (float)ki / 100;
      conkD = (float)kd / 100;
      if ( balancingOpt > 7 ) balancingOpt = 7;
      if ( balancingOpt < 0 ) balancingOpt = 0;
      switch (balancingOpt){
        case 0: 
                if ( balanceActive < 0 ) balanceActive = 0;
                if ( balanceActive > 2 ) balanceActive = 2;
                 if ( balanceActive == 0 ){
                  lcd.setCursor(1,0);
                  lcd.print("Balance: OFF    ");
                  Serial.println("Balance: OFF    ");
                  command = 0; 
                }  else
                if ( balanceActive == 1 ){
                  lcd.setCursor(1,0);
                  lcd.print("Balance: ON     ");
                  Serial.println("Balance: ON     "); 
                  command = 10;
                } else
                 if ( balanceActive == 2 ){
                  lcd.setCursor(1,0);
                  lcd.print("Balance: ERASE  ");
                  Serial.println("Balance: ERASE  ");
                  command = 11; 
                }  
                break;
        case 1: 
                if ( increasementType < 0 ) increasementType = 0;
                if ( increasementType > 2 ) increasementType = 2;
      ...
      if ( optionEnab )
        switch (balancingOpt){
          case 0: 
                buttonTiming(&buttonTimer, &buttonInc, &buttonDec, &balanceActive);
                break;
         case 1: 
...
/************** Load PID setting from EEPROM *****************/
void voidSaveSettings(){
  byte kpRound = ( conkP / 1 );
  byte kpFract = ( conkP - kpRound ) * 100;
  byte kiRound = ( conkI / 1 );
  byte kiFract = ( conkI - kiRound ) * 100;
  byte kdRound = ( conkD / 1 );
  byte kdFract = ( conkD - kdRound ) * 100;
  EEPROM.write(kpRound, 0);
  EEPROM.write(kpFract, 1);
  EEPROM.write(kiRound, 2);
  EEPROM.write(kiFract, 3);
  EEPROM.write(kdRound, 4);
  EEPROM.write(kdFract, 5);
  lcd.setCursor(1,1);
  lcd.print("Save OK!        "); 
  delay(50); 
}
...
/************** Load PID setting from EEPROM *****************/
void voidLoadSettings(){
  kp = int(EEPROM.read(0) + ( EEPROM.read(1) / 100 ));
  ki = int(EEPROM.read(2) + ( EEPROM.read(3) / 100 ));
  kd = int(EEPROM.read(4) + ( EEPROM.read(5) / 100 ));
  lcd.setCursor(1,1);
  lcd.print("Load OK!        "); 
  delay(50);   
}
