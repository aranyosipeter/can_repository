...
void setup(){
  ...
  pinMode(REDLEDO, OUTPUT);
  pinMode(GREENLEDO, OUTPUT);
  mirfInit();
  //Serial.begin(9600);
...
void loop(){
  ...
  if ( taskNum == 0 ){
           ...
           command = 0;
           while (( tries < 10 ) && ( dataRec == false )){           
             voidPacketSend(&command);
             timerValue = millis();
             while (( timerValue + 250 ) > millis() ){
             }
             tries++;
  ...
  if ( taskNum == 2 ){
           Serial.println("DB server: Communicate!");
           Mirf.channel = 80;
           Mirf.init();
           Mirf.config();
...
void voidGetPacket(){
  ...
  if ( receiveCommand == 0 ){
    bitWrite(relays, 0, rxbuff[2]);
    bitWrite(relays, 1, rxbuff[3]);
    bitWrite(relays, 2, rxbuff[4]);
    bitWrite(relays, 3, rxbuff[5]);
    bitWrite(relays, 4, rxbuff[6]);
    bitWrite(relays, 5, rxbuff[7]);
    bitWrite(relays, 6, rxbuff[8]);
    bitWrite(relays, 7, rxbuff[9]);
  }
  sendFlag = true;
  dataRec = true;
  digitalWrite(REDLED, HIGH);
  digitalWrite(REDLEDO, HIGH);
}
...
void transmissionLED(){
  ...
  if ( timer != prevTimer){
    ...
    if ( transmitNum > 5 ) digitalWrite(GREENLEDO, HIGH);
    if ( transmitNum <= 5 ) digitalWrite(GREENLEDO, LOW);
    transmitNum = 0;
    prevTimer = timer;
  }
}
