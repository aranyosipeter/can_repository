...
void slideController(){ 
  ...
  if (( rightSlide <= 384 ) && ( rightSlide >= 0 )){
    directRight = -1;
    acceRight = (( 384 - rightSlide ));
  }
  if (( leftSlide <= 384 ) && ( leftSlide >= 0 )){
    directLeft = -1;
    acceLeft = (( 384 - leftSlide ));
  }
  // Forward
  if (( rightSlide <= 1024 ) && ( rightSlide >= 640 )){
    directRight = 1;
    acceRight = (( rightSlide - 640 ));
  }
  if (( leftSlide <= 1024 ) && ( leftSlide >= 640 )){
    directLeft = 1;
    acceLeft = (( leftSlide - 640 ));
  }
  // Standby
...
void LEDControl(){
  ...
  if ( directRight == 1 ){
    analogWrite(RIGHT_ACC_LED, rightLEDs);
    analogWrite(RIGHT_DCC_LED, 0);
    digitalWrite(RIGHT_STD_LED, LOW);
  }
  if ( directRight == -1 ){
    analogWrite(RIGHT_ACC_LED , 0);
    analogWrite(RIGHT_DCC_LED , rightLEDs);
    digitalWrite(RIGHT_STD_LED , LOW);
  }
  ...
  if ( directLeft == 1 ){
    analogWrite(LEFT_ACC_LED, leftLEDs);
    analogWrite(LEFT_DCC_LED, 0);
    digitalWrite(LEFT_STD_LED, LOW);
  }
  if ( directLeft == -1 ){
    analogWrite(LEFT_ACC_LED, 0);
    analogWrite(LEFT_DCC_LED, leftLEDs);
    digitalWrite(LEFT_STD_LED, LOW);
  }
}
