...
unsigned long wallTiming = 0;
unsigned long planeTiming = 0;
unsigned long prevWallTiming = 0;
unsigned long prevPlaneTiming = 0
...
void setup()
{
  ...
  //attachInterrupt(button, buttonPush, RISING);
  //Serial.begin(9600);
}
...
void loopWall(){
  ...
  plainTiming = timing / 3;
    ...
    for(j=0;j<18000;j++){
      ...
      for(k=0;k<4;k++){
        digitalWrite(cubeOne[k],LOW);
        digitalWrite(cubeTwo[k],LOW);
      }
